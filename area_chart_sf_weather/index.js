...
Week 13 Lecture Demo - Area Chart on Historical Temperature in San Francisco
Baozhang Min
*/
...
const render = (data) => {
  ...
  */
  ...
  const xScale = d3
    ...
    .range([0, innerWidth]); // Remove nice() so that the area chart will not be cut off
  ...
  const xAxis = d3
    .axisBottom(xScale)
    .ticks(6) // Add more ticks to the x-axis
  ...
  // Create an area generator function with d3.js to create the line path
  const areaGenerator = d3
    .area()
    .x((d) => xScale(xValue(d)))
    .y0(innerHeight) // The bottom of the area chart
    .y1((d) => yScale(yValue(d))) // The top of the area chart
    .curve(d3.curveBasis); // Use curveBasis to create a smooth line

  // Create the line path using the line generator function
